...
unsigned char HDCPKeyProg[1024];
unsigned int HDCPKeyProgCount = 0;
unsigned char HDCPKeyRead[1024];
...
unsigned int addr_82D8 = 0x82D8;
unsigned int addr_32d8 = 0x332D8;
        ...
        for (i = 0; i < 0x100; i++) // ff
        {
                ...
                for (j = 0; j < 32; j++)
                {
                        if ((dwkey & (1 << j)) != 0)
                        {
                                ...
                                EfusePGMAddressRegValue.PGM_EFUSE_ADDR = ((j << 8) & 0x000001F00) + (i & 0x000000FF); // 0-31 is which bit;EfusePGMAddressRegValue.PGM_EFUSE_ADDR=((j<<8) & 0x0000001F00) +  (dword_addr & 0xFF);
...
void HdcpEfuseLock()
{
        ...
        EfusePGMAddressRegValue.Value = 0;
        EfusePGMAddressRegValue.PGM_EFUSE_REQ = 1;
        EfusePGMAddressRegValue.PGM_EFUSE_ADDR = 0x1C6D;  //bit[28]
        EfusePGMAddressRegMask.Value = 0xFFFFFFFF;
        WriteMMIOMask(addr_332DC, EfusePGMAddressRegValue.Value, EfusePGMAddressRegMask.Value);
...
int HdcpEfuseReadOneDWord(unsigned int dwaddr)
{
        ...
        unsigned int data;
        unsigned int data_32d8;
        dwaddr &= 0xff; // address max 0xff
        ...
        if (dwaddr <= 0x6D)
        {
                ...
                EfuseAddressRegMask.Value = 0x00FF0000;
                ...
                EfuseA_ReadModeRegMask.Value = 0x7e000000;
        ...
        EfuseAddressRegMask.Value = 0x00FF0000; // added changed
        WriteMMIOMask(addr_82B8, EfuseAddressRegValue.Value, EfuseAddressRegMask.Value);
        // printf("addr_82B8 is 0x%8.8x\n",addr_82B8);
        // printf("write_MMIO_82B8 is 0x%8.8x\n",ReadMMIO(addr_82B8,S3X_DWORD));
        // Send read command
        ...
        //data_32d8 = ReadMMIO(addr_32d8, S3X_DWORD);
       // printf("EfuseA_ReadRegValue.Value is :0x%8.8x",data_32d8);
        return data;
}
...
void HdcpEfuseLockCheck()
{

        REG_MM82B8 EfuseAddressRegValue, EfuseAddressRegMask;
        REG_MM82C0 EfuseA_ReadRegValue, EfuseA_ReadModeRegMask;
        REG_MM332DC EfusePGMAddressRegValue, EfusePGMAddressRegMask;
        unsigned int data,data_32d8,data_332dc;

        // 332dc[28:27]=2b'11;
        EfusePGMAddressRegValue.Value = 0;
        EfusePGMAddressRegValue.LOCK_BYTE_DEBUG = 0x3;
        EfusePGMAddressRegMask.Value = 0x18000000;
        WriteMMIOMask(addr_332DC, EfusePGMAddressRegValue.Value, EfusePGMAddressRegMask.Value);
        ...
        // read efuse data DWORD
        data_32d8 = ReadMMIO(addr_32d8, S3X_DWORD);
        //printf("EfuseA_ReadRegValue.Value is :0x%8.8x\n",data_32d8);
        //check 82d8[15]==1?0,0x6D 16bits;
        /*
        data_332dc = ReadMMIO(addr_332DC, S3X_DWORD);
        printf("1、mmio 332dc is :0x%8.8x\n",data_332dc);
        */
        if ((data_32d8 & 0x8000)== 0x0){
                printf("the efuse data[15:0] in address 0x6D is %4.4x\n",((data_32d8>>16) & 0xffff));
        }
        else if ((data_32d8 & 0x8000)== 0x8000){
                printf("the efuse data[31:16] in address 0x6D is %4.4x\n",((data_32d8>>16) & 0xffff));
        }
        ...
        //data_332dc = ReadMMIO(addr_332DC, S3X_DWORD);
        //printf("1、mmio 332dc is :0x%8.8x\n",data_332dc);
        //check 82d8[15]==1?0,0x6D 16bits;82d8 is diff from last;
        if ((data_32d8 & 0x8000)== 0x0){
                printf("the efuse data[15:0] in address 0x6D is %4.4x\n",((data_32d8>>16) & 0xffff));
        }
        else if ((data_32d8 & 0x8000)== 0x8000){
...
void HdcpEfuseProgramOneBit(unsigned int bit_addr)
{
        ...
        EfusePGMAddressRegValue.Value = 0;
        EfusePGMAddressRegValue.PGM_EFUSE_REQ = 1;
        EfusePGMAddressRegValue.PGM_EFUSE_ADDR = ((bit_addr << 8) & 0x000001F00) + ((bit_addr >> 5) & 0x000000FF);
        EfusePGMAddressRegMask.Value = 0xFFFFFFFF;
        WriteMMIOMask(addr_332DC, EfusePGMAddressRegValue.Value, EfusePGMAddressRegMask.Value);
...
void HdcpEfuseProgramOneByte(unsigned int byte_addr, unsigned char indata)
{
        ...
        for (j = 0; j < 32; j++)
        {
                if ((dwkey & (1 << j)) != 0)
                {
                        ...
                        EfusePGMAddressRegValue.Value = 0;
                        EfusePGMAddressRegValue.PGM_EFUSE_REQ = 1;
                        EfusePGMAddressRegValue.PGM_EFUSE_ADDR = ((j << 8) & 0x000001F00) + dwaddr; // 0-31 is which bit;
                        EfusePGMAddressRegMask.Value = 0xFFFFFFFF;
                        WriteMMIOMask(addr_332DC, EfusePGMAddressRegValue.Value, EfusePGMAddressRegMask.Value);
...
void HdcpEfuseProgramOneDWord(unsigned int dword_addr, unsigned int indata)
{
        ...
        for (j = 0; j < 32; j++) // indate:0x00000000  //burn which bit,which bit is 1;
        {
                if ((indata & (1 << j)) != 0) //[j->[0,31]],which bit;
                {
                        ...
                        EfusePGMAddressRegValue.Value = 0;
                        EfusePGMAddressRegValue.PGM_EFUSE_REQ = 1;
                        EfusePGMAddressRegValue.PGM_EFUSE_ADDR = ((j << 8) & 0x0000001F00) + (dword_addr & 0xFF); // turn to 12bits;->FF
                        EfusePGMAddressRegMask.Value = 0xFFFFFFFF;
                        WriteMMIOMask(addr_332DC, EfusePGMAddressRegValue.Value, EfusePGMAddressRegMask.Value);
                        do
                        {
...
void Usage()
{
        printf("Usage: Read or program CHX004 HDCP E-Fuse.\n");

        printf("efuse -p -i addr       #prog 1bit at bit addr(HEX)(000 - 1FFF).\n");
        printf("efuse -p -b addr data  #prog 8bit at byte addr(HEX)(000 - 3FF).\n");
        printf("efuse -p -w addr data  #prog 32bit at dword addr(HEX)(00 - FF).\n");
        //printf("efuse -p -f file.bin    #prog all data from a  binary file.\n");
        ...
        printf("efuse -r -i addr       #read 1bit at bit addr(HEX)(000 - 1FFF).\n");
        printf("efuse -r -b addr       #read 8bit at byte addr(HEX)(000 - 3FF).\n");
        printf("efuse -r -w addr       #read 32bit at dword addr(HEX)(00 - FF).\n");
        ...
        printf("efuse -d  #hdcp efuse debug whether lock-byte have burned any bit\n");
}
...
int main(int argc, char *argv[])
{
        ...
        if (argc == 2)
        {
                ...
                {
                        printf("HDCP EFuse lock check start...\n");
                        HdcpEfuseLockCheck();
                }
                else
                {
        ...
        else if (argc == 4)
        {
                ...
                if (strcmp(argv[1], "-P") == 0 || strcmp(argv[1], "-p") == 0)
                {
                        ...
                        else if (strcmp(argv[2], "-F") == 0 || strcmp(argv[2], "-f") == 0)
                        {
                                ...
                                HDCPKeyProgCount = fread(HDCPKeyProg, sizeof(unsigned char), 1024, in_file);
                                fclose(in_file);
                                HdcpEfuseProgramAll();
